...
const SurveyForm = ({ onComplete }: { onComplete: () => void }) => {
  ...
  if (submitted) {
    return (
      ...
      >
        ...
        <p className="text-slate-700">Suas respostas anônimas foram enviadas com sucesso e ajudarão em nossa pesquisa científica.</p>
        <p className="text-slate-500 text-sm mt-8 italic">Redirecionando para o início...</p>
      </motion.div>
    );
  }
  ...
  const RadioGroup = ({ name, options, label, onChange }: { name: string, options: string[], label: string, onChange?: (val: string) => void }) => (
    <div className="space-y-3">
      <label className="text-base font-bold text-slate-900 block">{label}</label>
    ...
    <form className="space-y-10" onSubmit={handleSubmit}>
        ...
        {influence === 'Outros' && (
          ...
          >
            <input 
              ...
              className="w-full px-6 py-4 rounded-2xl bg-slate-50 border border-slate-200 focus:border-vax-blue focus:ring-2 focus:ring-vax-blue/20 outline-none transition-all text-slate-900"
            />
          </motion.div>
        )}
      </div>
      ...
      <div className="space-y-6">
        ...
        {skippedVax === 'Sim' && (
          ...
          >
            <label className="text-sm font-bold text-slate-800 block">Se sim, por qual motivo?</label>
            <textarea 
              rows={3}
              onChange={(e) => setFormData((prev: any) => ({ ...prev, skipped_reason: e.target.value }))}
              className="w-full px-6 py-4 rounded-2xl bg-slate-50 border border-slate-200 focus:border-vax-blue focus:ring-2 focus:ring-vax-blue/20 outline-none transition-all resize-none text-slate-900"
            ></textarea>
          </motion.div>
        )}
      </div>
      ...
      <div className="space-y-2">
        <label className="text-base font-bold text-slate-900 block">10. Na sua opinião, por que algumas pessoas não querem se vacinar?</label>
        <textarea 
          rows={4}
